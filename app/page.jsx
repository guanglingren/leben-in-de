...
const JOBS = [
  { id: "shift", name: "仓库夜班", wage: 520, energy: -24, health: -6, stress: 7, abilityReq:0, socialReq:0, icon: "📦" },
  { id: "delivery", name: "外卖接单", wage: 450, energy: -17, health: -3, stress: 4, abilityReq:0, socialReq:0, icon: "🚲" },
  { id: "office", name: "大学办公室助理", wage: 590, energy: -16, health: -1, stress: 5, abilityReq:55, socialReq:45, icon: "🏫" },
  { id: "hiwi", name: "大学 HiWi 学生助理", wage: 630, energy: -14, health: 0, stress: 3, abilityReq:60, socialReq:50, icon: "🔬" },
  { id: "agency", name: "学生事务翻译", wage: 700, energy: -15, health: -1, stress: 5, abilityReq:72, socialReq:65, icon: "🗣️" }
];
...
const ACTIONS = [
  { id:"work", icon:"💼", name:"做本职工作", sub:"按当前职业结算工资", run:s=>{const job=JOBS.find(j=>j.id===s.jobId)||JOBS[0];const abilityBonus=Math.min(80,Math.max(0,integrationScore(s)-40)*2);const overdrawn=s.energy+job.energy<15||s.health<45;return {money:job.wage+abilityBonus+(s.flags.promotion?70:0),energy:job.energy,health:overdrawn?job.health:0,stress:job.stress,reputation:3};}},
...
export default function Home() {
  ...
  const ability=integrationScore(state),social=socialScore(state);
  const languageBonus=Math.min(80,Math.max(0,ability-40)*2);
  ...
  return <Localize lang={lang}><main className="v2-game">
    ...
    <section className="v2-panel">
      ...
      {tab==="actions"&&<div id="weekly-actions"><div className="panel-title decision-title"><div><small>WOCHE {state.totalWeek+1} · 还剩 {48-state.totalWeek} 周</small><h2>{state.marketVisitWeek===state.totalWeek?"这周，你选择了交易":"这周，把时间花在哪里？"}</h2></div><span>{state.marketVisitWeek===state.totalWeek?"完成交易后进入下一周":"五种方向，只能最终执行一种"}</span></div><div className="weekly-pressure"><b>必须在第 48 周前留下来</b><span>🎓 留德能力 {ability}/65</span><span>🏦 债务 {Math.round(state.debt)}/600€</span><em>{state.week===4?"⚠ 本周结束扣生活费 780€":"下一次月末结算还有 "+(5-state.week)+" 周"}</em></div><div className="rumor-card"><small>本周线索 · 会影响际遇和市场，但不保证准确</small><p>{lang==="de"?DE_NEWS[state.newsIndex]:news.text}</p></div><div className="featured-actions five-actions">{[...featuredChoices,tradeChoice].map(a=>{const isTrade=a.id==="browse-trade";return <button key={a.id} className={isTrade?"trade-choice":a.category==="本周际遇"?"temptation":""} onClick={()=>isTrade?openTrading():previewAction(a)} disabled={!isTrade&&((a.id==="work"&&state.energy<18)||state.marketVisitWeek===state.totalWeek)}><span className="choice-kind">{isTrade?"交易":a.category}</span><i>{a.icon}</i><b>{a.id==="work"?`去做 ${currentJob.name}`:a.name}</b><small>{a.id==="work"?`周薪约 ${currentWage}€；主要消耗精力`:a.sub}</small>{(a.risk||isTrade)&&<strong>{isTrade?a.risk:a.risk}</strong>}<em>{isTrade?(state.marketVisitWeek===state.totalWeek?"继续查看持仓 →":totalInventory>0?"查看持仓盈亏 →":"查看全部报价 →"):"先查看详情 →"}</em></button>})}</div><details className="more-actions"><summary>展开休息、看病与其他生活安排</summary><div className="action-grid">{ACTIONS.filter(a=>!["work","study","learn","social"].includes(a.id)).map(a=><button key={a.id} onClick={()=>a.planner?setModal({type:"paperPlanner"}):previewAction(a)} disabled={(a.id==="gig"&&state.energy<18)||state.marketVisitWeek===state.totalWeek}><i>{a.icon}</i><span><b>{a.name}</b><small>{a.sub}</small></span><em>先查看</em></button>)}</div></details><div className="month-cost"><b>查看不会推进时间</b><span>进入任一方案都可以返回；只有点击“确定执行”或完成一笔交易，才会锁定本周选择。</span></div></div>}
