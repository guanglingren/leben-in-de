...
const DE_GOODS = {
  bike:{name:"Gebrauchtes Fahrrad",note:"Bei Streiks und gutem Wetter besonders gefragt"},
  heater:{name:"Heizlüfter",note:"Bei einer Kältewelle schießt der Preis hoch"},
  ticket:{name:"Deutschlandticket aus dubioser Quelle",note:"Billig, aber Kontrolle und Personalisierung sind kein Spaß"},
  phone:{name:"Aufbereitetes Smartphone",note:"Für viele Online-Anträge fast unverzichtbar"},
  furniture:{name:"Kleiner IKEA-Tisch",note:"Zum Semesterstart steigt die Nachfrage"},
  printer:{name:"Drucker für zu Hause",note:"Für den nächsten Antrag brauchst du ihn bestimmt"},
  shoes:{name:"Gefälschte Markenschuhe",note:"Hohe Marge – aber eine Abmahnung kann teuer werden"},
  sticks:{name:"Manipulierter TV-Stick",note:"Gefragt, aber Plattform und Rechteinhaber schauen mit"}
};
...
function weeklyChoices(state){
  ...
  const progress=state.study<=state.german
    ? {...ACTIONS.find(a=>a.id==="study"),category:"补补短板",name:"把落下的课补回来",sub:"别让下次检查太难看"}
    : {...ACTIONS.find(a=>a.id==="learn"),category:"补补短板",name:"把德语再练顺一点",sub:"下回到窗口少卡壳"};
  const social={...ACTIONS.find(a=>a.id==="social"),category:"出去走走",name:"去混个脸熟",sub:"练德语，顺便认识人"};
  return [
    {...work,category:"挣钱 / 喘气"},
    progress,
    social
...
const momentKind=id=>id==="work"||id==="gig"?"work":id==="learn"||id==="study"||id==="tutorial"?"study":id==="rest"||id==="sport"?"rest":id==="trade-week"?"trade":id==="social"?"social":id?.startsWith("paper")?"paper":"life";
function GameMoment({kind="life",lang="zh",office="",entries=[]}){
  const config={
    work:{icon:"🧑‍🍳",prop:"📦",zh:"这一周，在工牌和时钟之间",de:"Diese Woche zwischen Stempeluhr und Schichtplan"},
    study:{icon:"🧑‍💻",prop:"📚",zh:"这一周，书页终于往后翻了",de:"Diese Woche ging es im Skript endlich weiter"},
    rest:{icon:"🛌",prop:"🌙",zh:"这一周，先把自己捡回来",de:"Diese Woche hast du erst einmal Kraft gesammelt"},
    trade:{icon:"🧑‍💼",prop:"📈",zh:"价格牌翻动，钱已经押上去了",de:"Die Kurse springen – dein Geld ist schon im Spiel"},
    social:{icon:"🧑‍🤝‍🧑",prop:"💬",zh:"这一周，有些事靠开口才推进",de:"Diese Woche ging es nur weiter, weil du gefragt hast"},
    paper:{icon:"🧍",prop:"🗂️",zh:"取号、等待、盖章，再等待",de:"Nummer ziehen, warten, Stempel – und weiter warten"},
    event:{icon:"🧍",prop:"🎫",zh:"德国日常突然插队",de:"Der deutsche Alltag funkt dazwischen"},
    life:{icon:"🧍",prop:"🧾",zh:"这一周留下了一张新账单",de:"Diese Woche hinterlässt eine neue Rechnung"}
  }[kind]||{};
  return <div className={`game-moment moment-${kind}`} aria-label={lang==="de"?config.de:config.zh}>
    <div className="moment-sky"><i/><i/><i/></div><span className="moment-person">{config.icon}</span><span className="moment-prop">{config.prop}</span>
    {kind==="event"&&<><span className="moment-ticket">{office||"AMT"}<b>042</b></span><span className="moment-stamp">{lang==="de"?"BITTE WARTEN":"请等待"}</span></>}
    {kind==="trade"&&<div className="moment-quotes"><i>€</i><i>↗</i><i>↘</i></div>}
    <small>{lang==="de"?config.de:config.zh}</small>
    {entries.length>0&&<div className="moment-effects">{entries.filter(item=>item.value!==0).map(item=><span className={item.value>=0?"gain":"cost"} key={item.key}>{item.icon} {item.value>0?"+":""}{item.value}{item.suffix||""}</span>)}</div>}
  </div>;
}
...
function Stat({label,value,type="bar",bad=false,onClick,hint}) {
  const content=<><span>{label}</span><b key={`${label}-${Math.round(value)}`} className="stat-number">{type==="money"?`${Math.round(value)} €`:Math.round(value)}</b>{hint&&<small>{hint}</small>}{type==="bar"&&<i><em style={{width:`${clamp(value)}%`}} className={bad?"danger":""}/></i>}</>;
...
function WeeklyMarket({state,lang,prices,onTrade}){
  ...
  const goodName=g=>lang==="de"?(DE_GOODS[g.id]?.name||g.name):g.name;
  const goodNote=g=>lang==="de"?(DE_GOODS[g.id]?.note||g.note):g.note;
  return <section className="weekly-market" id="weekly-market">
    <div className="weekly-market-head"><div><small>{lang==="de"?"WOCHENMARKT · HANDELSPLATTFORM":"WOCHENMARKT · 统一交易平台"}</small><h3>{lang==="de"?"Alle Angebote auf einen Blick":"全部商品，一次看完"}</h3></div><span>{lang==="de"?`Lager ${totalQty}/${state.capacity}`:`储物 ${totalQty}/${state.capacity}`}</span></div>
    ...
    <p className="market-freedom">{lang==="de"?(locked?"Du hast diese Woche bereits gehandelt. Damit steht Handel als Wochenaktion fest. Du kannst deinen Bestand noch anpassen und danach die Woche abschließen.":"Preise ansehen kostet keine Zeit. Erst mit dem ersten Kauf oder Verkauf wird Handel zu deiner Wochenaktion."):(locked?"本周已经成交：交易就是本周行动。可以继续调整货物，完成后推进一周。":"浏览全部报价不会推进时间。第一笔成交后，“交易”会成为本周行动。")}</p>
    {holdings.length>0&&<section className="portfolio-spotlight" id="portfolio-spotlight"><header><div><small>{lang==="de"?"MEIN BESTAND":"MEIN BESTAND · 我的持仓"}</small><h4>{lang==="de"?"Was sind deine Sachen heute wert?":"手上的货，现在值多少？"}</h4></div><span>{lang==="de"?"Alles verkaufen":"全部卖出"} <b className={holdingValue-holdingCost>=0?"profit":"loss"}>{holdingValue-holdingCost>=0?"+":""}{Math.round(holdingValue-holdingCost)}€</b></span></header><div>{holdings.map(g=>{const qty=state.inventory[g.id],avg=(state.inventoryCost[g.id]||0)/qty,sell=Math.round(prices[g.id]*.9),profit=Math.round((sell-avg)*qty);return <article key={g.id}><i>{g.icon}</i><span><b>{goodName(g)} × {qty}</b><small>{lang==="de"?"Ø-Kaufpreis":"买入均价"} <strong>{Math.round(avg)}€</strong> → {lang==="de"?"Verkauf heute":"现在卖出"} <strong>{sell}€</strong></small></span><em className={profit>=0?"profit":"loss"}>{profit>=0?"+":""}{profit}€</em><button onClick={()=>onTrade(g,"sell",qty,"platform")}>{lang==="de"?"Alles verkaufen":"全部卖出"}</button></article>})}</div></section>}
    <div className="trade-explain">💡 {lang==="de"?"Waren mit geringem Risiko schwanken weniger. Hohe mögliche Gewinne bedeuten meist unruhigere Preise – und manchmal Ärger mit Plattform oder Behörden.":"低风险商品波动较小；高回报商品通常价格更不稳定，并可能遭遇平台审核或查处。"}</div>
...
function TradeLedger({state,lang,prices}){
  const holdings=GOODS.filter(g=>(state.inventory?.[g.id]||0)>0);
  const translatedGood=name=>DE_GOODS[GOODS.find(g=>g.name===name)?.id]?.name||name;
  const marketName=id=>id==="platform"?"Handelsplattform":({campus:"Campus und Wohnheim",kleinanzeigen:"Kleinanzeigen",flohmarkt:"Wochenend-Flohmarkt"}[id]||"Handelsplattform");
  return <section className="trade-ledger-panel"><div className="panel-title"><div><small>HANDELSBUCH</small><h2>{lang==="de"?"Lager und Handelsbuch":"库存与交易账本"}</h2></div><span>{lang==="de"?"Kaufpreis → heutiger Verkaufswert":"成本 → 当前卖出价值"}</span></div><p className="market-tip">{lang==="de"?"Gehandelt wird direkt vor der Wochenaktion. Hier siehst du nur Bestand, Einstandspreise und abgeschlossene Geschäfte.":"买卖入口已经放进“本周行动”。这里专门查看库存成本、当前潜在盈亏和最近成交记录。"}</p>
    <div className="holding-grid">{holdings.length?holdings.map(g=>{const qty=state.inventory[g.id],avg=(state.inventoryCost[g.id]||0)/qty,sell=Math.round(prices[g.id]*.9),profit=Math.round((sell-avg)*qty);return <article key={g.id}><i>{g.icon}</i><div><b>{lang==="de"?(DE_GOODS[g.id]?.name||g.name):g.name}</b><small>{lang==="de"?`${qty} Stück · Ø ${Math.round(avg)} € · Verkauf heute ${sell} €`:`${qty} 件 · 均价 ${Math.round(avg)}€ · 当前卖价 ${sell}€`}</small></div><em className={profit>=0?"profit":"loss"}>{profit>=0?"+":""}{profit}€</em></article>}):<div className="empty">{lang==="de"?"Noch kein Bestand. Wähle unter „Wochenaktion“ zuerst einen Markt.":"目前没有库存。请先在“本周行动”里选择本周市场。"}</div>}</div>
    <div className="trade-history"><div className="subhead"><b>{lang==="de"?"Letzte Geschäfte":"最近成交"}</b><span>{lang==="de"?"Neue Einträge zuerst":"最新在前"}</span></div>{(state.tradeLedger||[]).length?(state.tradeLedger||[]).slice(0,12).map((entry,index)=><p key={index}><span>{lang==="de"?(entry.mode==="buy"?"Kauf":"Verkauf"):(entry.mode==="buy"?"买入":"卖出")} {lang==="de"?translatedGood(entry.good):entry.good}<small>{lang==="de"?marketName(entry.market):(entry.market==="platform"?"统一交易平台":MARKETS.find(m=>m.id===entry.market)?.name||"交易平台")} · {entry.qty} {lang==="de"?"Stück":"件"} × {entry.unitPrice}€</small></span><b>{entry.mode==="sell"&&entry.result!==null?`${entry.result>=0?"+":""}${entry.result}€`:`-${entry.qty*entry.unitPrice}€`}</b></p>):<div className="empty">{lang==="de"?"Noch keine Geschäfte.":"还没有成交记录。"}</div>}</div>
  </section>;
}
...
export default function Home() {
  ...
  function doAction(action,force=false){
    ...
    const outcomeEntries=coreEffectEntries(state,{...actionEffect,energy:(actionEffect.energy||0)+10});
    if(event&&shouldEvent)setModal({type:"event",event,timeLabel,actionName:action.name,actionId:action.id,monthSummary,teaser,outcomeEntries});
    else setModal({type:"weekResult",actionName:action.name,actionId:action.id,timeLabel,monthSummary:`${monthSummary||""} ${teaser}`.trim(),outcomeEntries});
  }
  ...
  return <Localize lang={lang}><main className="v2-game">
    ...
    <section className="v2-panel">
      ...
      {tab==="actions"&&<div id="weekly-actions"><div className="panel-title decision-title"><div><small>WOCHE {state.totalWeek+1} · {lang==="de"?`${48-state.totalWeek} übrig`:`还剩 ${48-state.totalWeek} 周`}</small><h2>{state.marketVisitWeek===state.totalWeek?(lang==="de"?"Diese Woche: Handel":"这周，你选择了交易"):(lang==="de"?"Wofür nimmst du dir Zeit?":"这周，把时间花在哪里？")}</h2></div></div><div className="weekly-pressure"><b>{lang==="de"?"Ziel: 48 Wochen schaffen":"目标：撑过 48 周"}</b><span>🎓 {lang==="de"?"Perspektive":"能力"} {ability}/65</span><span>🏦 {Math.round(state.debt)}/600€</span>{state.week===4&&<em>⚠ {lang==="de"?`${BASE_MONTHLY_COST} € Lebenshaltungskosten`:`本周扣生活费 ${BASE_MONTHLY_COST}€`}</em>}</div><div className="rumor-card"><small>{lang==="de"?"GERÜCHT · OHNE GEWÄHR":"本周线索 · 未必准确"}</small><p>{lang==="de"?DE_NEWS[state.newsIndex]:news.text}</p></div><div className="featured-actions five-actions">{[...featuredChoices,tradeChoice].map(a=>{const isTrade=a.id==="browse-trade";return <button key={a.id} className={isTrade?"trade-choice":""} onClick={()=>isTrade?openTrading():previewAction(a)} disabled={!isTrade&&state.marketVisitWeek===state.totalWeek}><span className="choice-kind">{isTrade?(lang==="de"?"MARKT":"淘货"):a.category}</span><i>{a.id==="work"&&state.energy<35?"🛌":a.icon}</i><b>{a.id==="work"?state.energy<35?(lang==="de"?"Erst einmal durchatmen":"先歇口气吧"):(lang==="de"?"Arbeiten oder Pause?":"上班，还是歇一周"):a.name}</b><small>{a.id==="work"?(state.energy<18?(lang==="de"?"Heute geht keine Schicht mehr.":"这状态干不了活"):(lang==="de"?`Etwa ${currentWage} € – oder Kraft sparen.`:`约 ${currentWage}€，或留点力气`)):a.sub}</small>{a.risk&&<strong>{a.risk}</strong>}<em>{isTrade?(state.marketVisitWeek===state.totalWeek?(lang==="de"?"Bestand prüfen →":"再看持仓 →"):totalInventory>0?(lang==="de"?"Gewinn oder Verlust? →":"赚了还是赔了？→"):(lang==="de"?"Zum Markt →":"去市场 →")):(lang==="de"?"Ansehen →":"看看 →")}</em></button>})}</div><details className="more-actions"><summary>{lang==="de"?"Weitere Möglichkeiten":"其他生活安排"}</summary><div className="action-grid">{ACTIONS.filter(a=>!["work","rest","study","learn","social"].includes(a.id)).map(a=><button key={a.id} onClick={()=>a.planner?setModal({type:"paperPlanner"}):previewAction(a)} disabled={(a.id==="gig"&&state.energy<18)||state.marketVisitWeek===state.totalWeek}><i>{a.icon}</i><span><b>{a.name}</b><small>{a.sub}</small></span><em>{lang==="de"?"Ansehen":"看看"}</em></button>)}</div></details><div className="month-cost"><b>{lang==="de"?"Danach kann noch etwas dazwischenkommen.":"之后，生活还可能插一脚。"}</b></div></div>}
    ...
    {modal&&!modal.minimized&&modal.type!=="debtPay"&&<div className="modal-backdrop"><article className={`event-modal ${modal.type==="event"?"event-arrival":modal.type==="weekResult"?"week-turn":"result-arrival"}`}>
      {modal.type==="actionPreview"?<><div className="modal-top"><span>{modal.action.category||"生活安排"}</span><b>本周方案 · 尚未执行</b></div><div className="decision-state optional"><b>现在只是查看方案</b><span>可以返回比较其他选择；确认后才会推进一周。状态预估已经计入每周自然恢复。</span></div><GameMoment kind={momentKind(modal.action.id)} lang={lang}/><h2>{modal.action.id==="work"?`这周去挣钱，还是先喘口气？`:modal.action.name}</h2><p>{modal.action.id==="work"?`去做 ${currentJob.name}，大概能拿 ${currentWage}€。要是已经撑不住，少赚一周也比把身体拖垮强。`:modal.action.sub}</p><div className="action-effect-preview core-effect-preview">{coreEffectEntries(state,previewEffect(modal.action)).map(item=><span key={item.key}><small>{item.icon} {lang==="de"?item.de:item.zh}</small><b className={item.value>=0?"profit":"loss"}>{item.value>0?"+":""}{item.value}{item.suffix||""}</b></span>)}</div>{modal.action.risk&&<div className="preview-risk">{modal.action.risk}</div>}<button className="primary" disabled={modal.action.id==="work"&&state.energy<18} onClick={()=>doAction(modal.action,true)}>{modal.action.id==="work"?(state.energy<18?"这状态真上不了班":"去上班，推进一周"):"就这么过这一周"} <span>→</span></button>{modal.action.id==="work"&&<button className="recovery-choice" onClick={()=>doAction(restAction,true)}><b>🛌 这周先不硬撑</b><span>状态明显恢复 · 花费 18€</span><em>睡够了再说 →</em></button>}<button className="secondary" onClick={()=>setModal(null)}>← 返回比较其他选择</button></>:modal.type==="paperPlanner"?<><div className="modal-top"><span>AKTIONSWAHL</span><b>行动方案 · 尚未执行</b></div><div className="decision-state optional"><b>现在只是查看方案</b><span>尚未消耗时间，可以返回改选工作、学习、休息或交易。</span></div><GameMoment kind="paper" lang={lang}/><h2>这周具体跑什么手续？</h2><p>只有选择下面一项，才会把“处理手续”确定为本周行动并推进一周。</p><div className="paper-tasks">{PAPER_TASKS.map(task=><button key={task.id} onClick={()=>doPaperTask(task)}><b>{task.icon} {task.name}</b><span>{task.sub}</span><em>{coreEffectText(state,task.effect,lang)}</em></button>)}</div><button className="secondary" onClick={()=>setModal(null)}>← 返回本周选择 · 不推进时间</button></>:modal.type==="event"?<><div className="modal-top"><span>{modal.event.office}</span><b>周末事件 · 已经发生</b></div><div className="decision-state required"><b>本周行动已经完成</b><span>这个事件必须处理；可以暂时收起查看状态，但不能改选本周行动。</span></div><GameMoment kind="event" lang={lang} office={modal.event.office} entries={modal.outcomeEntries}/><div className="time-passed">{modal.timeLabel}<small>{modal.actionName}已经用掉一周</small></div><h2>{modal.event.title}</h2><p>{modal.event.text}</p><div className="modal-choices">{modal.event.choices.map(c=><button key={c.label} onClick={()=>chooseEvent(c,modal.event)}><b>{c.label}</b><span>{coreEffectText(state,c.effect||{},lang)}</span></button>)}</div><button className="secondary inspect-state" onClick={()=>setModal({...modal,minimized:true})}>暂时收起查看状态 · 之后仍需处理</button></>:modal.type==="weekResult"?<><div className="modal-top"><span>WOCHENABSCHLUSS</span><b>周结算</b></div><GameMoment kind={momentKind(modal.actionId)} lang={lang} entries={modal.outcomeEntries}/><div className="time-passed large">{modal.timeLabel}<small>时间已经推进</small></div><h2>这一周结束了</h2><p>{modal.actionName}已经完成。{modal.monthSummary||"新的市场价格和生活状态已经更新。"}</p><button className="primary" onClick={()=>setModal(null)}>进入新的一周 <span>→</span></button></>:modal.type==="ventureResult"?<><div className="modal-top"><span>NEBENGEWERBE</span><b>经营结算</b></div><GameMoment kind="trade" lang={lang}/><div className="holding-period">持有 {modal.ledger.heldWeeks} 周后结算<small>开始和结算本身均不耗时间</small></div><h2>{modal.title}</h2><p>{modal.result}</p><div className="venture-result-numbers"><span>总投入<b>{modal.ledger.invested}€</b><small>本金 {modal.ledger.capital}€ + 渠道 {modal.ledger.fee}€</small></span><span>回收金额<b>{modal.ledger.returned}€</b><small>实际回到账户的价值</small></span><span>净利润<b className={modal.ledger.profit>=0?"profit":"loss"}>{modal.ledger.profit>=0?"+":""}{modal.ledger.profit}€</b><small>回收 − 总投入</small></span></div><button className="primary" onClick={()=>setModal(null)}>完成结算 <span>→</span></button></>:<><div className="modal-top"><span>AUSWIRKUNG</span><b>处理结果</b></div><GameMoment kind="event" lang={lang} office={modal.event?.office} entries={modal.outcomeEntries}/>{modal.timeLabel&&<div className="time-passed">{modal.timeLabel}<small>事件发生在已经消耗的这一周</small></div>}<h2>{modal.choice.label}</h2><p>{modal.choice.result}</p>{modal.languageRelief>0&&<div className="language-relief">状态损失因为你的语言能力有所减轻</div>}<button className="primary" onClick={()=>setModal(null)}>进入新的一周 <span>→</span></button></>}
    </article></div>}
  </main></Localize>;
}
