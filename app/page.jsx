...
function WeeklyMarket({state,lang,prices,onTrade,onFinish}){
  ...
  return <section className="weekly-market" id="weekly-market">
    ...
    })}</div>{locked&&<div className="trade-commit"><span><small>本周行动已确定</small><b>统一平台交易</b></span><button onClick={onFinish}>完成交易，推进一周 →</button></div>}
  </section>;
}
...
export default function Home() {
  ...
  const tradeChoice={id:"browse-trade",icon:"🛒",name:"去交易平台看看",sub:"浏览全部商品与本周报价；查看不消耗时间",risk:"第一笔成交后，本周行动确定为交易"};
  ...
  return <Localize lang={lang}><main className="v2-game">
    ...
    <section className="v2-panel">
      ...
      {tab==="actions"&&<div id="weekly-actions"><div className="panel-title decision-title"><div><small>WOCHE {state.totalWeek+1} · 还剩 {48-state.totalWeek} 周</small><h2>{state.marketVisitWeek===state.totalWeek?"这周，你选择了交易":"这周，押哪一边？"}</h2></div><span>{state.marketVisitWeek===state.totalWeek?"完成交易后揭晓":"每周选择一种主要行动"}</span></div><div className="weekly-pressure"><b>必须在第 48 周前留下来</b><span>🎓 学业 {state.study}/65</span><span>🏦 债务 {Math.round(state.debt)}/600€</span><em>{state.week===4?"⚠ 本周结束扣生活费 780€":"下一次月末结算还有 "+(5-state.week)+" 周"}</em></div><div className="rumor-card"><small>本周线索 · 消息可能是真的，也可能已经过时</small><p>{lang==="de"?DE_NEWS[state.newsIndex]:news.text}</p></div><div className="featured-actions">{[...featuredChoices,tradeChoice].map((a,index)=>{const isBet=a.id.startsWith("bet-"),isTrade=a.id==="browse-trade";return <button key={a.id} className={isBet?"temptation":index===0?"safe":isTrade?"trade-choice":""} onClick={()=>isTrade?document.getElementById("weekly-market")?.scrollIntoView({behavior:"smooth",block:"start"}):a.planner?setModal({type:"paperPlanner"}):doAction(a)} disabled={!isTrade&&((a.id==="work"&&state.energy<18)||state.marketVisitWeek===state.totalWeek)}><span className="choice-kind">{isTrade?"交易":isBet?"机会":index===0?"保底":"成长"}</span><i>{a.icon}</i><b>{a.id==="work"?`去做 ${currentJob.name}`:a.name}</b><small>{a.id==="work"?`确定拿到约 ${currentWage}€，但会消耗大量精力`:a.sub}</small><strong>{isTrade?a.risk:isBet?a.risk:index===0?"结果较确定 · 保住基本盘":"稳定推进 · 放弃本周赚钱机会"}</strong><em>{isTrade?(state.marketVisitWeek===state.totalWeek?"继续调整交易 →":"查看全部报价 →"):state.marketVisitWeek===state.totalWeek?"本周已选择交易":"押这一周 →"}</em></button>})}</div><details className="more-actions"><summary>展开其他生活安排</summary><div className="action-grid">{ACTIONS.filter(a=>!featuredChoices.some(f=>f.id===a.id)).map(a=><button key={a.id} onClick={()=>a.planner?setModal({type:"paperPlanner"}):doAction(a)} disabled={(a.id==="work"||a.id==="gig")&&state.energy<18||state.marketVisitWeek===state.totalWeek}><i>{a.icon}</i><span><b>{a.name}</b><small>{a.sub}</small></span><em>⏳ 推进1周</em></button>)}</div></details><div className="month-cost"><b>查看不等于选择</b><span>打开交易平台和比较报价不消耗时间；工作、学习、休息或第一笔成交才会确定本周行动。</span></div></div>}
      {tab==="actions"&&<>{state.marketVisitWeek===state.totalWeek&&<div className="trade-selected-action"><i>🛒</i><span><small>本周行动已锁定</small><b>你选择了交易</b><p>可以继续调整货物；完成后将推进一周并刷新行情。</p></span><button onClick={()=>doAction(TRADE_WEEK_ACTION)}>完成交易 →</button></div>}<WeeklyMarket state={state} lang={lang} prices={prices} onTrade={trade} onFinish={()=>doAction(TRADE_WEEK_ACTION)}/></>}
