...
function WeeklyMarket({state,lang,prices,previewMarket,onPreview,onTrade,onFinish}){
  ...
  return <section className="weekly-market" id="weekly-market">
    ...
    <p className="market-freedom">{locked?"本周已经成交：交易就是本周行动。可以继续调整同一市场的货物，完成后推进一周。":"可以查看所有市场，随时返回。只有实际成交后，才会把“交易”确定为本周行动。"}</p>
    ...
    })}</div>{locked&&<div className="trade-commit"><span><small>本周行动已确定</small><b>在 {chosen.name} 交易</b></span><button onClick={onFinish}>完成交易，推进一周 →</button></div>}</>}
  </section>;
}
...
export default function Home() {
  ...
  return <Localize lang={lang}><main className="v2-game">
    ...
    <section className="v2-panel">
      ...
      {tab==="actions"&&<div id="weekly-actions"><div className="panel-title decision-title"><div><small>WOCHE {state.totalWeek+1} · 还剩 {48-state.totalWeek} 周</small><h2>{state.marketVisitWeek===state.totalWeek?"这周，你选择了交易":"这周，押哪一边？"}</h2></div><span>{state.marketVisitWeek===state.totalWeek?"完成交易后揭晓":"行动与交易二选一"}</span></div><div className="weekly-pressure"><b>必须在第 48 周前留下来</b><span>🎓 学业 {state.study}/65</span><span>🏦 债务 {Math.round(state.debt)}/600€</span><em>{state.week===4?"⚠ 本周结束扣生活费 780€":"下一次月末结算还有 "+(5-state.week)+" 周"}</em></div><div className="rumor-card"><small>本周线索 · 消息可能是真的，也可能已经过时</small><p>{lang==="de"?DE_NEWS[state.newsIndex]:news.text}</p></div><div className="featured-actions">{featuredChoices.map((a,index)=>{const isBet=a.id.startsWith("bet-"),riskClass=isBet?(a.risk.startsWith("高")?"risk-high":a.risk.startsWith("中")?"risk-medium":"risk-low"):"";return <button key={a.id} className={`${isBet?"temptation":index===0?"safe":""} ${riskClass}`} onClick={()=>a.planner?setModal({type:"paperPlanner"}):doAction(a)} disabled={(a.id==="work"&&state.energy<18)||state.marketVisitWeek===state.totalWeek}><span className="choice-kind">{isBet?"机会":index===0?"保底":"成长"}</span><i>{a.icon}</i><b>{a.id==="work"?`去做 ${currentJob.name}`:a.name}</b><small>{a.id==="work"?`确定拿到约 ${currentWage}€，但会消耗大量精力`:a.sub}</small><strong>{isBet?a.risk:index===0?"结果较确定 · 保住基本盘":"稳定推进 · 放弃本周赚钱机会"}</strong><em>{state.marketVisitWeek===state.totalWeek?"本周已选择交易":"押这一周 →"}</em></button>})}</div><details className="more-actions"><summary>不想押？展开其他生活安排</summary><div className="action-grid">{ACTIONS.filter(a=>!featuredChoices.some(f=>f.id===a.id)).map(a=><button key={a.id} onClick={()=>a.planner?setModal({type:"paperPlanner"}):doAction(a)} disabled={(a.id==="work"||a.id==="gig")&&state.energy<18||state.marketVisitWeek===state.totalWeek}><i>{a.icon}</i><span><b>{a.name}</b><small>{a.sub}</small></span><em>⏳ 推进1周</em></button>)}</div></details><div className="month-cost"><b>每周只能选择一种主要行动</b><span>工作、学习、休息或交易都会推进一周；查看行情和比较方案不消耗时间。</span></div></div>}
      {tab==="actions"&&<><div className="plan-bridge"><b>交易也是一种本周行动</b><span>查看行情：免费，可返回</span><i>→</i><span>实际成交：本周选择交易</span></div>{state.marketVisitWeek===state.totalWeek&&<div className="trade-selected-action"><i>🛒</i><span><small>本周行动已锁定</small><b>你选择了交易</b><p>可以继续调整货物；完成后将推进一周并刷新行情。</p></span><button onClick={()=>doAction(TRADE_WEEK_ACTION)}>完成交易 →</button></div>}<ProgressBenefits state={state} lang={lang}/><WeeklyMarket state={state} lang={lang} prices={prices} previewMarket={previewMarket} onPreview={setPreviewMarket} onTrade={trade} onFinish={()=>doAction(TRADE_WEEK_ACTION)}/></>}
