...
export default function Home() {
  ...
  return <Localize lang={lang}><main className="v2-game">
    ...
    <section className="v2-panel">
      ...
      {tab==="actions"&&<><div className="plan-bridge"><b>交易也是一种本周行动</b><span>查看全部商品：免费，可返回</span><i>→</i><span>实际成交：本周选择交易</span></div>{state.marketVisitWeek===state.totalWeek&&<div className="trade-selected-action"><i>🛒</i><span><small>本周行动已锁定</small><b>你选择了交易</b><p>可以继续调整货物；完成后将推进一周并刷新行情。</p></span><button onClick={()=>doAction(TRADE_WEEK_ACTION)}>完成交易 →</button></div>}<WeeklyMarket state={state} lang={lang} prices={prices} onTrade={trade} onFinish={()=>doAction(TRADE_WEEK_ACTION)}/></>}
